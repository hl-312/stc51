...
void Int_DigitalTubes_Init(struct Int_DigitalTubes_Struct *this)
{
    uint8_t i;
    this->type             = CATHODE;
    this->segmentCodeTable = s_segmentCodeTable;
    this->setting[0]       = 0xff;
    this->setting[1]       = 0x00;
    for (i = 0; i < 8; i++) {
        this->buffer[i] = i;
...
void Int_DigitalTubes_DisplayOne(Int_DigitalTubes_Struct *this, uint8_t site)
{
    if (this->setting[0] & 0x01 << site) // 检查位开关
    {
        ...
        if (this->setting[1] & 0x01 << site)                                                                                               // 检查小数点开关
            SEM = this->type ? ~(this->segmentCodeTable[this->buffer[site]] | 0x80) : (this->segmentCodeTable[this->buffer[site]] | 0x80); // 赋值显示内容，有小数点所以或一下0x80
        else
            SEM = this->type ? ~(this->segmentCodeTable[this->buffer[site]]) : (this->segmentCodeTable[this->buffer[site]]); // 无小数点，直接赋值
    }
    return;
}
